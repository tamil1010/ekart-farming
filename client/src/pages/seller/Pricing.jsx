...
const generateMarketPrice = (basePrice) => {
...
const SellerPricing = () => {
  const { user } = useAuth();
  const { products: allProducts, editProduct } = useData();
  const [searchTerm, setSearchTerm] = useState('');
  
  const products = allProducts.filter(p => p.sellerId === user?.id);
  ...
  const marketPrices = useMemo(() => {
    return products.reduce((acc, p) => {
      ...
      const seed = String(p._id || p.id || '').split('').reduce((a, b) => a + b.charCodeAt(0), 0);
  ...
  const handleUpdatePrice = (specialPrice) => {
    ...
    editProduct(targetProduct._id || targetProduct.id, { 
      price: numericPrice,
  ...
  const getSuggestedPrice = (marketPrice) => {
  ...
  const filteredProducts = products
    ...
    .sort((a, b) => {
      ...
      if (sortBy === 'diff') {
        const diffA = (marketPrices[a._id || a.id]?.price || 0) - a.price;
        const diffB = (marketPrices[b._id || b.id]?.price || 0) - b.price;
        return diffB - diffA;
      }
      return 0;
    });
  ...
  const getPriceStatus = (sellerPrice, marketPrice) => {
  ...
  const criticalAlerts = products.filter(p => {
    const market = marketPrices[p._id || p.id];
          ...
          <div className="flex flex-col md:flex-row gap-4">
             ...
             <div className="flex gap-2">
                <div className="relative">
                   ...
                   >
                      {categories.map(cat => <option key={cat || 'default'} value={cat}>{cat || 'Uncategorized'}</option>)}
                   </select>
                ...
                <table className="w-full text-left">
                   <thead className="bg-bg-surface/50 text-text-secondary text-[10px] uppercase font-black tracking-widest">
                      <tr>
                         ...
                         <th className="px-6 py-4">Suggested Rate</th>
                   ...
                   <tbody className="divide-y divide-brand-dark/10">
                      {filteredProducts.map(product => {
                        const market = marketPrices[product._id || product.id];
                        if (!market) return null;
                        ...
                        return (
                          <tr key={product._id || product.id} className="group hover:bg-white/5 transition-colors">
                             ...
                             <td className="px-6 py-5 text-center">
                                <div className={`text-sm font-black ${product.stock < 10 ? 'text-red-400' : 'text-text-secondary'}`}>
               ...
               <div className="p-8 text-white">
                 ...
                 <div className="grid grid-cols-3 gap-4 mb-8">
                    <div className="p-4 bg-bg-surface/50 rounded-2xl border border-brand-dark/20 text-center">
                       ...
                       <p className="text-2xl font-black text-white">₹{marketPrices[editingProduct._id || editingProduct.id]?.price || 0}</p>
                    </div>
                    <div className="p-4 bg-bg-surface/50 rounded-2xl border border-brand-dark/20 text-center relative group transition-all hover:border-emerald-500/30">
                       ...
                       <p className="text-2xl font-black text-emerald-500 italic">₹{getSuggestedPrice(marketPrices[editingProduct._id || editingProduct.id]?.price || 0)}</p>
                       <button 
                         onClick={() => handleUpdatePrice(getSuggestedPrice(marketPrices[editingProduct._id || editingProduct.id]?.price || 0))}
                 ...
                 <div className="space-y-2">
                    ...
                    <div className="flex justify-between items-center px-1">
                       <p className="text-[10px] text-text-secondary font-bold">Recommended: ₹{(marketPrices[editingProduct._id || editingProduct.id]?.price || 0) - 10} - ₹{(marketPrices[editingProduct._id || editingProduct.id]?.price || 0) + 10}</p>
